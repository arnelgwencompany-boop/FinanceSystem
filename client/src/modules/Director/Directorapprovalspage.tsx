...
import {
  ...
  ChevronLeft, ChevronRight, Loader2, Inbox, Shield,
} from "lucide-react";
import {
...
const ROLE = "director" as const;
const USER = { name: "Ricardo Bautista", initials: "RB" };

// Director acts on: Supervisor Approved only
const canAct = (status: string) => status === "Supervisor Approved";
...
export default function DirectorApprovalsPage() {
  ...
  const handleApprove = (id: number) => {
    setActing(true);
    setTimeout(() => {
      setRequests((p) => p.map((r) => r.id === id ? { ...r, status: "Fully Approved" as const } : r));
      addToast("success", "Request fully approved and forwarded to Finance for disbursement.");
      setActing(false);
    }, 700);
  };
  ...
  const TABS: FilterTab[] = ["All", "Supervisor Approved", "Fully Approved", "Pending", "Rejected"];
    ...
    <main className="ml-[270px] mt-16 min-h-[calc(100vh-64px)] bg-[#f7f9fb] overflow-y-auto">
      ...
      <div className="px-8 py-8 max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[15px] text-slate-500 mt-0.5">
                Review Supervisor-approved requests and issue final approval for disbursement.
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3 px-4 py-2.5 rounded-xl border-2 border-teal-200 bg-teal-50">
            ...
            <div>
              ...
              <p className="text-[12px] text-teal-600 font-semibold">Director</p>
            </div>
          </div>
        </div>

        {/* Summary cards — director focuses on Supervisor Approved */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <SummaryCard label="Awaiting You"  count={counts["Supervisor Approved"]} active={activeTab === "Supervisor Approved"} onClick={() => { setActiveTab("Supervisor Approved"); setPage(1); }} icon={<Clock size={20} className="text-blue-600" />}           color="bg-blue-50"    />
          <SummaryCard label="Fully Approved"count={counts["Fully Approved"]}      active={activeTab === "Fully Approved"}      onClick={() => { setActiveTab("Fully Approved");      setPage(1); }} icon={<CheckCheck size={20} className="text-emerald-600" />}   color="bg-emerald-50" />
        ...
        {/* Actionable notice */}
        {counts["Supervisor Approved"] > 0 && (
          <div className="flex items-center gap-3 px-5 py-3.5 rounded-xl border border-blue-200 bg-blue-50">
            <CheckCircle2 size={18} className="text-blue-600 flex-shrink-0" />
            <p className="text-[15px] font-semibold text-blue-800">
              <span className="font-extrabold">{counts["Supervisor Approved"]}</span> request{counts["Supervisor Approved"] > 1 ? "s" : ""} approved by the Supervisor and awaiting your final approval.
            </p>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          {/* Tabs — Director's primary focus is "Supervisor Approved" so it's listed first */}
          <div className="flex border-b border-slate-100 overflow-x-auto">
            {TABS.map((tab) => (
              <button key={tab} onClick={() => { setActiveTab(tab); setPage(1); }}
                ...
                {tab === "Supervisor Approved" ? "Awaiting My Approval" : tab}
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-slate-50">
                  {paginated.map((r) => {
                    ...
                    return (
                      <tr key={r.id} className={`transition-colors hover:bg-slate-50 ${act ? "bg-blue-50/40" : ""}`}>
